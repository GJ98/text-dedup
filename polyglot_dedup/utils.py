...
def add_io_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    """
    parser.add_argument(
        "--type", type=str, help="data `type`", required=True)
    parser.add_argument(
        "--data_files", type=str, help="`data_files` in load_dataset"),
    parser.add_argument(
        "--split", type=str, default='train', help="`split` in load_dataset"),
    parser.add_argument(
        "--cache_dir", type=str, help="`cache_dir` in load_dataset"),
    parser.add_argument(
        "--output_dir", type=str, help="Path to save all results", required=True),
    parser.add_argument(
        "--index_name", type=str, help="Name of the index file, will be saved in `output_dir`"),
    parser.add_argument(
        "--reuse_index", action=argparse.BooleanOptionalAction, help="Reuse the index if already exists"),
    parser.add_argument(
        "--graph_name", type=str, help="Name of the cluster file, will be saved in `output_dir`"),
    parser.add_argument(
    ...
    #parser.add_argument("--path", type=str, help="`path` of data", required=True),
    ...
    #parser.add_argument("--use_auth_token", action=argparse.BooleanOptionalAction, help="`use_auth_token` in load_dataset"),
...
def add_meta_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--column", 
        type=str, 
        help="""Text column to use for deduplication. If multiple columns are desired,
...
def add_minhash_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="""Ngram size to use in MinHash. The tokenization is space-based,
                you can modify it by modifying the `ngrams` function in `utils.py`""",
    ),
    parser.add_argument(
        "--seed", type=int, default=42, help="Seed to use in MinHash"),
    parser.add_argument(
        "--num_perm", type=int, default=128, help="Number of permutations to use in MinHash"),
    parser.add_argument(
